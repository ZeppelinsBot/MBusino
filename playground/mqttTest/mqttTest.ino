...
//char telegram[769] = {0}; // only for tests
...
void loop() {
  ...
  // === MQTT STRESS TEST ===
  static unsigned long stressMsgSent = 0;
  ...
  if (client.connected() && millis() - stressLastMsg >= 2) {
    ...
    if (client.publish("mbusino/stress-test", p)) {
      stressMsgSent++;
    }
  }

  if (millis() - stressLastStats >= 10000) {
    stressLastStats = millis();
    Serial.printf("[Stress] sent: %lu\n", stressMsgSent);
    stressMsgSent = 0;
  }
  // === END STRESS TEST ===
