...
// Test telegram: almost all zeros (header + 190 zero bytes + checksum + stop)
// Used to isolate corruption patterns on the bus
const uint8_t TEST_TELEGRAM[] = {
  0x68,0xC1,0xC1,0x68,0x08,0x00,0x72,  // header (L=193, C=08, A=00, CI=72)
  // 190 zero bytes (CI is already part of header above, so 190 data bytes)
  ...
  0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
  0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
  0x7A,  // checksum: sum(bytes 4..196) = 0x08+0x72 = 0x7A
  0x16   // stop
};
